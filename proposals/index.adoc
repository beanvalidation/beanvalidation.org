= Jakarta Validation Next proposals
Gunnar Morling
:awestruct-layout: default

This page contains a list of proposals of changes that could be included into the next Jakarta Validation specification.
Feedback is very welcome on our https://accounts.eclipse.org/mailing-list/bean-validation-dev[mailing list].

Check out the following issues on GitHub to learn more on what is currently discussed and considered for Jakarta Validation:

- https://github.com/jakartaee/validation/validation-spec[Jakarta Validation specification issues]
- https://github.com/jakartaee/validation/issues[Jakarta Validation API issues]
- https://github.com/jakartaee/validation-tck/issues[Jakarta Validation TCK issues]

[[proposal-table]]
[cols="25%,75%"]
|===
|Issue Id (GitHub) |Description
...
<script>
    ...
    const issues = document.createElement('tbody');
    table.append(issues);
    fetch(`https://api.github.com/repos/jakartaee/validation-spec/issues`)
      .then(res => res.json())
      .then(data => {
        data.forEach(issue => {
          const tr = document.createElement("tr");
          tr.innerHTML = `<td class="tableblock halign-left valign-top"><p class="tableblock"><a href="${issue.html_url}">Spec: ${issue.number}</a></p></td><td class="tableblock halign-left valign-top"><p class="tableblock">${issue.title}</p></td>`;
          issues.appendChild(tr);
        });
        fetch(`https://api.github.com/repos/jakartaee/validation/issues`)
            .then(res => res.json())
            .then(data => {
                data.forEach(issue => {
                const tr = document.createElement("tr");
                tr.innerHTML = `<td class="tableblock halign-left valign-top"><p class="tableblock"><a href="${issue.html_url}">API: ${issue.number}</a></p></td><td class="tableblock halign-left valign-top"><p class="tableblock">${issue.title}</p></td>`;
                issues.appendChild(tr);
            });
        })
        .catch(err => {
      ...
      .catch(err => {
        console.error(err);
        issues.innerHTML = "Failed to load issues.";
      });
  </script>
+++
