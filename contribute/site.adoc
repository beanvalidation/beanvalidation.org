...
Emmanuel Bernard
:description: Instructions for contributing to the beanvalidation.org website
:sectanchors:
...
in http://asciidoc.org/[AsciiDoc], a powerful and intuitive markup language.
...
* Get Ruby
* If on macOS, get Xcode (needed for native gems)
...
* Run `python -m http.server 4242`

You should be back to millisecond serving :)

== Markup samples

You can find sample files in the Git repository itself for both `.adoc` and `.html.haml` syntaxes. Look at them, especially the `.adoc` files, as they show how a page should be written and how to use syntax highlighting.

Menus are in `_layout`. When you create a page, give it the right metadata in the document header:

* `title` (The first line starting with `=`)
* `author` (The second line)
* `:layout:` (Optional attribute)
* `:page-comments:` (Optional attribute)

When you set the `:page-comments:` property to `true`, a Disqus comment section is added at the bottom of the page.
